...
class MemoryCard(CardWidget):
    ...
    def init_ui(self):
        ...
        if self.memory.context:
            ...
            if "chapter" in self.memory.context:
                context_texts.append(tr("galgame.dialog.memory.chapter_format", default="第{}章").format(self.memory.context['chapter']))
            if context_texts:
    ...
    def _get_type_name(self) -> str:
        type_names = {
            "interaction": tr("galgame.dialog.memory.type_interaction", default="互动"),
            "promise": tr("galgame.dialog.memory.type_promise", default="约定"),
            "gift": tr("galgame.dialog.memory.type_gift", default="礼物"),
            "conflict": tr("galgame.dialog.memory.type_conflict", default="冲突"),
            "special": tr("galgame.dialog.memory.type_special", default="特殊")
        }
...
class CharacterMemoryTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.total_label = BodyLabel(tr("galgame.dialog.memory.total_format", default="总计: {total}条记忆").format(total=summary['total']), self)
...
class MemoryDialog(QDialog):
    def __init__(self, memory_manager: CharacterMemoryManager, character_names: List[str], parent=None):
        ...
        self.setWindowTitle(tr("galgame.dialog.memory.title", default="记忆回顾"))
    ...
    def init_ui(self):
        ...
        self.title_label = TitleLabel("💭 " + tr("galgame.dialog.memory.title", default="记忆回顾"), self)
        layout.addWidget(self.title_label)

        self.desc_label = BodyLabel(tr("galgame.dialog.memory.description", default="查看角色们记得的事情，这些都是你们共同经历的回忆。"), self)
        ...
        self.close_btn = PushButton(tr("galgame.dialog.memory.close", default="关闭"), self)
    ...
    def refresh_ui(self):
        self.setWindowTitle(tr("galgame.dialog.memory.title", default="记忆回顾"))
        self.title_label.setText("💭 " + tr("galgame.dialog.memory.title", default="记忆回顾"))
        self.desc_label.setText(tr("galgame.dialog.memory.description", default="查看角色们记得的事情，这些都是你们共同经历的回忆。"))
        self.close_btn.setText(tr("galgame.dialog.memory.close", default="关闭"))

        # Refresh tab summary labels
        for i in range(self.tab_widget.count()):
            tab = self.tab_widget.widget(i)
            if isinstance(tab, CharacterMemoryTab):
                summary = tab.memory_manager.get_memory_summary(tab.character_name)
                total_text = tr("galgame.dialog.memory.total_format", default="总计: {total}条记忆").format(total=summary['total'])
                tab.total_label.setText(total_text)
